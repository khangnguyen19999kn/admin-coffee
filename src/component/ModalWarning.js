...
export default function ModalWarning({ item, index}) {
  const {renderList,resetList} = useContext(MenuContext)

  const deleteProduct = ()=>{
    let promise = Axios({
      url: `https://api-coffee-phen.herokuapp.com/api/v1/product/${item.id}`,
      method: 'DELETE'
  ...
  return (
    <div>
       ...
       <div class="modal" id={"exampleDeletePro" + index} tabindex="-1" role="dialog">
